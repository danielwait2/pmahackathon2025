...
export function TeamAvailability({
  ...
  const overlapCounts: number[][] = Array.from({ length: 7 }, () =>
    Array(48).fill(0),
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            {compact ? "Availability Heatmap" : "Team Availability Heatmap"}
          </CardTitle>
          ...
          <div className="overflow-x-auto">
            <div className="inline-block min-w-full border border-slate-200 rounded-lg overflow-hidden">
              ...
              <div className="grid grid-cols-8">
                <div className="bg-slate-50 border-b border-r border-slate-200 p-2 text-xs font-medium text-slate-600">
                  Time
                </div>
                {DAYS_OF_WEEK.map((day, idx) => (
                  <div
                    key={day}
                    className={`bg-slate-50 border-b border-slate-200 p-2 text-xs font-medium text-center text-slate-900 ${idx < 6 ? "border-r" : ""}`}
                  >
                ...
                {DISPLAY_HALF_HOURS.map((halfHour) => {
                  ...
                  return (
                    <React.Fragment key={halfHour}>
                      ...
                      <div
                        className={`h-4 bg-white text-xs text-slate-600 flex items-center border-r border-slate-200 ${isFullHour ? "p-1 font-bold border-t-2 border-t-slate-300" : ""}`}
                      >
                      ...
                      {DAYS_OF_WEEK.map((_, day) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() =>
                              handleCellClick(day, halfHour, count)
                            }
